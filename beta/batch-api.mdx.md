...
| portfolioDocTypes            | Array of types of documents to extract from, if all the documents in the batch are [portfolio](/layout-based-extractions/portfolio) documents.      |
...
3. Put each document to its upload URL to request extraction, using the same guidelines as you'd use for the [generate_upload_url](/api-reference/extraction/document/generate-upload-url) endpoint.  For example:
...
4. Retrieve each extraction using the [documents/{extraction_id}](/api-reference/extraction/retrieve-extractions/retrieving-results) endpoint. 
...
5. You can generate an Excel file that combines the completed extractions in the batch by using the batch ID. For example:
   ...
   For more information about converting extractions to Excel, see [SenseML to spreadsheet reference](/integrations/quick-extraction/excel-reference).
