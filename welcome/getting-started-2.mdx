...
* [**Integrate**](/welcome/integrating) using Sensible's API, SDKs, quick-extract UI, or other tools
* [**Quality control**](/best-practices/validating-extractions) extracted data
* [**Monitor**](/best-practices/monitoring-extractions) extracted data in production
...
* Select a new example bank statement:  
   * If you use one of your own bank statement, ensure it's one that combines multiple accounts. Note that Sensible [supports](/layout-based-extractions/out-of-the-box-extractions) extracting data from a single-account bank statement, but it's outside the scope of this tutorial.  
...
Now you've published your config, you can get document data at scale using Sensible's APIs, SDKs, or bulk-upload UI. Put the extracted data to work in Excel files, databases, and other destinations. For more inforamtion, see [Integrating](/welcome/integrating).

### Next: Learn more about extraction

* See [prompt authoring tips](/llm-based-extractions/prompt-tips/overview)
* For advanced extraction strategies, see [Choosing an extraction approach](/welcome/choosing-an-extraction-approach)