...
| whole-document table recognition       | Avoid configuring Sensible to search a whole document for tables. Instead, configure a table stop. For examples, see any of the table [methods](https://sensible-87.mintlify.app/senseml-reference/concepts/choosing-a-table-method).                       |

**Under 5 seconds per document**

| Factor                                          | Notes                                                                                                                                                                                                                                              |
| ----------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| OCR preprocessor                                | Some documents mix digital text with text images, for example by embedding scanned pages in a digital PDF. Speed this up by OCRing select pages, not the whole document. For more information, see the [OCR preprocessor](https://sensible-87.mintlify.app/senseml-reference/preprocessors/ocr-preprocessor). |
| Fixed Tables with stops,Nearest Checkbox method | Sensible processes Fixed Tables that include a Stop parameter in less than 5 seconds. Or, convert to the faster [Text Table](https://sensible-87.mintlify.app/senseml-reference/methods/text-table) method, which skips table recognition.                                                              |
| LLM-based methods                               | For methods based on large language models (LLMs), see [Sensible Instruct](https://sensible-87.mintlify.app/llm-based-extractions/prompt-tips/overview)                                                                                                                                                         |
...
| Signature method, Checkbox method,[image coordinate extraction](https://sensible-87.mintlify.app/senseml-reference/methods/document-range) | These methods have no alternatives. See the following section for ways to avoid running these methods except when necessary. |

### Document type performance

By default, Sensible runs all the configs in a document type before choosing the best one for a given document. If your document type contains configs with computationally expensive methods, you can improve performance by selectively running and skipping configs. Use fingerprints to test whether documents contain matching text before skipping or running configs. For more information, see [fingerprint](https://sensible-87.mintlify.app/senseml-reference/config-settings/fingerprint).