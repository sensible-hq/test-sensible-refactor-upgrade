...
In contrast, Sensible converts document tables, checkboxes, paragraphs, and even complex repeating section layouts into meaningfully labeled column/row pairs and linked sheets. For a list of document file types that Sensible can extract data from, see [Supported file types](https://sensible-87.mintlify.app/senseml-reference/concepts/supported-file-types).
...
* You extract from a [portfolio](https://sensible-87.mintlify.app/layout-based-extractions/multi-document-extractions) document.
* You input multiple extractions to the Generate Excel [method](https://sensible-87.mintlify.app/integrations/sdk-guides) or [endpoint](https://sensible-87.mintlify.app/api-reference/extraction/get-excel-from-documents/get-excel-extraction).
...
![](/images/multi-sheet-fields.png)
 In the preceding example, Sensible recommends using either `car_year` or `manufacture_year`, not both.

* To configure multi-document spreadsheet output using different rules than those described here, configure rules with Sensible's [Zapier](https://sensible-87.mintlify.app/integrations/zapier-overview/overview) integration. You can map single-value field output with this integration, but not multi-value field output.
...
* outputs a single `value`. For example, the [Box](https://sensible-87.mintlify.app/senseml-reference/methods/box), [Label](https://sensible-87.mintlify.app/senseml-reference/methods/label), [Row](https://sensible-87.mintlify.app/senseml-reference/methods/row) and [Region](https://sensible-87.mintlify.app/senseml-reference/methods/region) methods each output a single value by default.
* outputs a predictably short array of values that can easily be stringified in a single cell. Typically this is the result of a [Type](https://sensible-87.mintlify.app/senseml-reference/field-query-object/types) configuration, for example, the [Name](https://sensible-87.mintlify.app/senseml-reference/field-query-object/types#name) type always outputs an array.

### Example

Sensible converts the JSON extraction output from the [auto\_insurance\_anyco](https://github.com/sensible-hq/sensible-docs/raw/main/readme-sync/assets/v0/pdfs/auto%5Finsurance%5Fanyco.pdf) example described in the [Getting started with layout-based extractions](https://sensible-87.mintlify.app/welcome/getting-started) to the following spreadsheet:

**Spreadsheet output**

![](/images/spreadsheet-output.png)
**Example document**
...
See the [Getting started with layout-based extractions](https://sensible-87.mintlify.app/welcome/getting-started) for the SenseML configuration for this example.
...
* the [table methods](https://sensible-87.mintlify.app/senseml-reference/concepts/choosing-a-table-method) and other methods that output nested JSON objects.
* methods that output arrays of unpredictable length, for example, fields with `"match":"all"` configured.

### Example

Sensible converts the example JSON output from the [example document](https://raw.githubusercontent.com/sensible-hq/sensible-docs/main/readme-sync/assets/v0/pdfs/fixed%5Ftable.pdf) described in the [Fixed Table](https://sensible-87.mintlify.app/senseml-reference/methods/fixed-table#examples) method to the following spreadsheet:
...
| Example document | [Download link](https://raw.githubusercontent.com/sensible-hq/sensible-docs/main/readme-sync/assets/v0/pdfs/table%5Fdynamic.pdf) |
| ---------------- | -------------------------------------------------------------------------------------------------------------------------------- |

**Example configuration**

See [Fixed Table](https://sensible-87.mintlify.app/senseml-reference/methods/fixed-table#examples) method for the SenseML configuration for this example.
...
Each `<field_id>.<index>` sheet lists the output of a single field that contains complex repeating data, for example, [Sections](https://sensible-87.mintlify.app/senseml-reference/sections/overview) output.

### Example

Sensible converts the example JSON output from the [example document](https://raw.githubusercontent.com/sensible-hq/sensible-docs/main/readme-sync/assets/v0/pdfs/vertical%5Fsection%5Ftable%5Fin%5Ftable.pdf) described in the [Advanced Sections nested table example](https://sensible-87.mintlify.app/senseml-reference/sections/advanced--nested-table-example) topic to the following spreadsheet:

![](/images/example-table2.png)
...
| Example document | [Download link](https://raw.githubusercontent.com/sensible-hq/sensible-docs/main/readme-sync/assets/v0/pdfs/vertical%5Fsection%5Ftable%5Fin%5Ftable.pdf) |
| ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |

**Example configuration**

See the [Advanced sections nested table example](https://sensible-87.mintlify.app/senseml-reference/sections/advanced--nested-table-example) topic for the SenseML configuration for this example.
...
* For a tutorial about converting document data to an Excel file using the Sensible app, see [Getting started with out-of-the-box extraction](https://sensible-87.mintlify.app/layout-based-extractions/out-of-the-box-extractions).
* To get started with extracting from your custom documents, see [Getting Started](https://sensible-87.mintlify.app/welcome/getting-started).