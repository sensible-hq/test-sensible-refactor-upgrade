...
This Sensible Instruct method extracts individual facts in a document, such as the date of an invoice, the liability limit of an insurance policy, or the destination address of a shipping container delivery. When you configure the Multimodal Engine parameter, this method can extra data from non-text images, such as photographs, charts, or illustrations. For an example, see [Example: Extract from images](https://sensible-87.mintlify.app/senseml-reference/llm-based-methods/query-group#example-extract-from-images)

Sensible recommends grouping queries together if they share [context](https://sensible-87.mintlify.app/senseml-reference/llm-based-methods/query-group#notes). Queries share context when data exists in the same location or region of a document, for example, on the same page. You can configure context using [Advanced prompt configuration](https://sensible-87.mintlify.app/llm-based-extractions/advanced-prompt-configuration). 
...
For the full reference for this method in SenseML, see [Query group method](https://sensible-87.mintlify.app/senseml-reference/llm-based-methods/query-group).