...
| Operation                               | PDF | Microsoft Word (\DOC and \DOCX) | Microsoft Excel(XLSX) | image formats(JPEG, PNG, and TIFF) |
| --------------------------------------- | --- | ----------------------------- | --------------------- | ---------------------------------- |
...
* When extracting from image file formats, Sensible ignores OCR or OCR preprocessor settings you configure in the document type or SenseML configuration. For more information about OCR, see [OCR level](https://sensible-87.mintlify.app/senseml-reference/document-type-settings/ocr-level).
