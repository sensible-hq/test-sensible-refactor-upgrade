...
**Note:** If you're familiar with Sensible, this detailed topic is for you. If you're new to Sensible, see [sections](https://sensible-87.mintlify.app/senseml-reference/sections/overview).
...
1. Finds the range, or y-extent, for a section in the group using Match and Stop lines. Sections’ ranges never overlap, and ranges can span pages. In detail:  
   * If there's _no_ Stop parameter, the current section's range stops above the next anchor's Match + Y Offset parameters.  
   * If there's an _optional_ Stop parameter, the current section stops either:  
         * above the next anchor's Match + Y Offset parameter, _or_  
         * below the next Stop line + Stop Y Offset parameter.  
         * Use an _optional_ stop to prevent the last section in the group from extending to the end of the document.  
   * If there's a _required_ Stop, the next section stops below the next Stop line + Stop Y Offset parameter, and Sensible ignores any intervening anchor matches.
...
**Tip:** To extract repeated vertical section groups, nest them in a parent section group. For an example, see [Advanced: nested columns example](https://sensible-87.mintlify.app/senseml-reference/sections/advanced--nested-columns-example).
...
* "Kept" columns: If you configure the Column Selection parameter, Sensible adds the lines in any _unspecified_ columns to each section.
...
For more information about this example, see [Labeled rows and labeled columns table example](https://sensible-87.mintlify.app/senseml-reference/sections/labeled-rows-example)
...
In this case, Sensible creates sections by splitting text into "rows" at each newline. For more information about the behavior shown in the following image, see the [Zip sections example](https://sensible-87.mintlify.app/senseml-reference/sections/advanced--zip-sections).

![Click to enlarge](/images/sections_match_all_anchors.png)

For vertical sections, specifying the regular expression `".+"` for the Match parameter can be useful for repeating vertical sections that lack good anchor match candidates. In these situations, you must also nest the section group in a parent section group. For more information, see [Table grid example](https://sensible-87.mintlify.app/senseml-reference/sections/advanced--table-grid-example).