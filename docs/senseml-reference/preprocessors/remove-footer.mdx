...
* (Default) Sensible searches for repeated text at the bottom of the page. For more information about automatic recognition, see [Notes](https://sensible-87.mintlify.app/senseml-reference/preprocessors/remove-footer#notes).
...
| startsOnPage        | integer. default: 1                                   | The first page number on which to start checking for repeated elements. Note this is the page _number_, not the page's zero-based index in the pages array. To filter out end pages that lack a repeating element, use the Page Range preprocessor to define an End Page parameter.                                                 |
| match               | [Match](https://sensible-87.mintlify.app/senseml-reference/field-query-object/match-object) object or array of Match objects | Bypasses automatic footer recognition.Removes all text on the page below the bottom boundary of the matched line.If Sensible doesn't find the match, it doesn't perform footer removal.                                                                                                                                             |
| offsetY             | number in inches. default: 0                          | Bypasses automatic footer recognition.Defines a point at which to start text removal. Positive values offset down the page, negative values offset up the page.If used with no Match parameter defined, offsets from the bottom of the page.If used with the Match parameter, offsets from the bottom boundary of the matched line. |
...
![Click to enlarge](/images/remove_footer_1.png)

![Click to enlarge](/images/remove_footer_2.png)

| Example document | [Download link](https://raw.githubusercontent.com/sensible-hq/sensible-docs/main/readme-sync/assets/v0/pdfs/remove%5Ffooter.pdf) |
| ---------------- | -------------------------------------------------------------------------------------------------------------------------------- |
...
These elements are _not_ recognized as "repeating": 
