...
Enables transforming a table by adding an array of computed fields that operate on the extracted table. For example, you can add text to each cell in a row, concatenate columns, add or remove columns, and so forth. This method is similar to adding a [computed fields array to sections](https://sensible-87.mintlify.app/senseml-reference/sections/advanced--transform-sections-data). In detail, this method:

* Automatically [zips](https://sensible-87.mintlify.app/senseml-reference/computed-field-methods/zip) the source table. After the zip, each table cell is a field whose ID is the column heading. Note that zipped tables have the same JSON output structure as sections. This enables you to use section-specific computed fields to transform the table, such as the Copy To Section method.
...
| source\_id (**required**) | source field ID in the current config                    | The ID of the table you want to transform. To transform fields in a section, add a [computed fields array to sections](https://sensible-87.mintlify.app/senseml-reference/sections/advanced--transform-sections-data).                                                                                                   |
| fields                    | array of [computed fields](https://sensible-87.mintlify.app/senseml-reference/computed-field-methods/overview) | Specifies to output computed fields in the new table. The computed fields have access to the source table's fields. To get access to and transform the output of fields that aren't in the source table, use the [Copy to section](https://sensible-87.mintlify.app/senseml-reference/advanced-computed-field-methods/copy-to-section) method. |
