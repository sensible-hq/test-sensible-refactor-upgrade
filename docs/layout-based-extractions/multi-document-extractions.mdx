...
Sensible recommends extracting each document in a portfolio using its own document type, so you can write [validations](https://sensible-87.mintlify.app/best-practices/validating-extractions) for each type. For example, use an "income tax" doc type and an "invoice" doc type, rather than creating a "combined\_tax\_and\_invoice" doc type.

To extract from a portfolio, take the following steps:

* Specify [fingerprints](https://sensible-87.mintlify.app/senseml-reference/config-settings/fingerprint) to configure how Sensible segments the portfolio into documents. Fingerprints test for text matches on first pages, last pages, and other page types.
...
The config is the same as the one used in the [Getting started with layout-based extractions](https://sensible-87.mintlify.app/welcome/getting-started), with the addition of the following fingerprint:
...
The config is the same as the one used in the [Sections](https://sensible-87.mintlify.app/senseml-reference/sections/overview) topic, with the addition of the following fingerprint:
...
```
2. This request returns an extraction ID. Use it to retrieve the extractions by replacing \`_YOUR\_EXTRACTION\_ID_ with the returned ID in the following example code:
