...
Specifies fields to exclude from the extraction output. For example, if you use the field ID `_raw_data` as a source for a [computed field](/senseml-reference/computed-field-methods/index), then specify the raw field's ID in this method to show only the computed field in the output.
