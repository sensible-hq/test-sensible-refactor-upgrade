...
* [Field query object](/senseml-reference/field-query-object/index)
* [Preprocessors](/senseml-reference/preprocessors/index)
* [Methods](/senseml-reference/methods/index)
* [LLM-based methods](/senseml-reference/llm-based-methods/index), including LLM-based Sensible Instruct methods. For more information about choosing whether to author configs in either SenseML or Sensible Instruct, see [Choosing an extraction approach](/welcome/author).
* [Configuration settings](/senseml-reference/config-settings/index)
* [Computed Field methods](/senseml-reference/computed-field-methods/index)
* [Sections](/senseml-reference/sections/index)
...
* The **field** `_driver_name_raw` is a query that extracts a driver's name by searching below some matched text (`"position": "below"`). Its ID is the key for the extracted data. For more information, see [Field query object](/senseml-reference/field-query-object/index).
...
|                                              | [LLM-based methods](/senseml-reference/llm-based-methods/index)                                                                                                |  Layout-based [methods](/senseml-reference/methods/index)                                                                                                                              |  
| -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |  
...
* The **preprocessor**, `pageRange`, cuts out irrelevant pages of the document. For more information about using preprocessors to clean up documents before extracting data, see [Preprocessors](/senseml-reference/preprocessors/index).
* The **fingerprint** tells Sensible to preferentially run this config if the document contains the terms "anyco" or "quoted coverage changes." For more information about using fingerprints to improve performance, and other configuration settings, see [Configuration Settings](/senseml-reference/config-settings/index).
* The **computed field** `"driver_name_last"` extracts the last name from the raw output of the `_driver_name_raw` field. For more information about transforming field output, see [Computed field methods](/senseml-reference/computed-field-methods/index). You can also capture the full name as typed output. See [types](/senseml-reference/field-query-object/types).
