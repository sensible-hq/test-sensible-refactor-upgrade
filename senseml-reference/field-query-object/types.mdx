...
Filter and format extracted data using the Type parameter in a [Field object](/senseml-reference/field-query-object/overview). 
...
* As a more syntactically concise alternative to the [Regex](/senseml-reference/methods/regex) method or to [Computed Field methods](/senseml-reference/computed-field-methods/overview). For example, you can write a field to capture a date-typed field, then transform the field's output with the [Split](/senseml-reference/computed-field-methods/split) method. Or, see the following example to transform dates using the Compose type.
...
Required when you define a [table method](/senseml-reference/concepts/choosing-a-table-method).
