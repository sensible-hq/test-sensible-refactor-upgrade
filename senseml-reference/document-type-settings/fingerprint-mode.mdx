...
A [fingerprint](/senseml-reference/config-settings/fingerprint) for standalone documents changes Sensible's default behavior of running _all_ the configs in a single document type. For example, if you extract company A and company B quotes, by default Sensible runs both the company A and the company B configs for a given document, then returns the extraction with the highest score. 
...
The classification score is for comparing extractions within a single document type. To compare scores across document types, see [Accuracy measures](/senseml-reference/concepts/accuracy-measures).
