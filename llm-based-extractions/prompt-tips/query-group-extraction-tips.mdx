...
Sensible recommends grouping queries together if they share [context](/senseml-reference/llm-based-methods/query-group#notes). Queries share context when data exists in the same location or region of a document, for example, on the same page. You can configure context using [Advanced prompt configuration](/llm-based-extractions/advanced-prompt-configuration). 
