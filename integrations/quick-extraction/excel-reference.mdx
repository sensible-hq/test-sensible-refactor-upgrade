...
* To configure multi-document spreadsheet output using different rules than those described here, configure rules with Sensible's [Zapier](/integrations/zapier/index) integration. You can map single-value field output with this integration, but not multi-value field output.
...
Each `<field_id>.<index>` sheet lists the output of a single field that contains complex repeating data, for example, [Sections](/senseml-reference/sections/index) output.
