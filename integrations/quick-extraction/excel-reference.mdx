...
* You extract from a [portfolio](/layout-based-extractions/portfolio) document.
* You input multiple extractions to the Generate Excel [method](/integrations/sdk-guides) or [endpoint](/api-reference/extraction/get-excel-from-documents/get-excel-extraction).
...
<iframe width="700" src="https://docs.google.com/spreadsheets/d/e/2PACX-1vRkj6u0ukgEr9VpHP19vuzohqtqJYJOOtzYzNGi3ZFJYJQ3hLVBowJDjbTcjeBahZoz9_rULfFO3Hu6/pubhtml?widget=true&amp;headers=false"></iframe>

 In the preceding example, Sensible recommends using either `car_year` or `manufacture_year`, not both.

* To configure multi-document spreadsheet output using different rules than those described here, configure rules with Sensible's [Zapier](/integrations/zapier/index-zapier) integration. You can map single-value field output with this integration, but not multi-value field output.
...
* outputs a single `value`. For example, the [Box](/senseml-reference/methods/box), [Label](/senseml-reference/methods/label), [Row](/senseml-reference/methods/row) and [Region](/senseml-reference/methods/region) methods each output a single value by default.
* outputs a predictably short array of values that can easily be stringified in a single cell. Typically this is the result of a [Type](/senseml-reference/field-query-object/types) configuration, for example, the [Name](/senseml-reference/field-query-object/types#name) type always outputs an array.

### Example

Sensible converts the JSON extraction output from the [auto\_insurance\_anyco](https://github.com/sensible-hq/sensible-docs/raw/main/readme-sync/assets/v0/pdfs/auto%5Finsurance%5Fanyco.pdf) example described in the [Getting started with layout-based extractions](/welcome/getting-started-ai) to the following spreadsheet:

**Spreadsheet output**

<iframe width="700" src="https://docs.google.com/spreadsheets/d/e/2PACX-1vRJO_nwPRVe84ZdAi-gc6mny0zhRO9iz4nclfEKSBFQWHotARcgUkwfcinpGJTzPM4GIoIvf6PcN7zv/pubhtml?widget=true&amp;headers=false"></iframe>
...
See the [Getting started with layout-based extractions](/welcome/getting-started-ai) for the SenseML configuration for this example.
...
* the [table methods](/senseml-reference/concepts/table-methods) and other methods that output nested JSON objects.
* methods that output arrays of unpredictable length, for example, fields with `"match":"all"` configured.

### Example

Sensible converts the example JSON output from the [example document](https://raw.githubusercontent.com/sensible-hq/sensible-docs/main/readme-sync/assets/v0/pdfs/fixed%5Ftable.pdf) described in the [Fixed Table](/senseml-reference/methods/fixed-table#examples) method to the following spreadsheet:

<iframe width="700" src="https://docs.google.com/spreadsheets/d/e/2PACX-1vRU-wjC2m6F0ACgU1Ry_W1zggeb7-oM_tZZ7XAsadz8m7uzZos1TY7B0XLpGW79_7rgltqpj8eCB262/pubhtml?widget=true&amp;headers=false"></iframe>
...
See [Fixed Table](/senseml-reference/methods/fixed-table#examples) method for the SenseML configuration for this example.
...
Each `<field_id>.<index>` sheet lists the output of a single field that contains complex repeating data, for example, [Sections](/senseml-reference/sections/index-sections) output.

### Example

Sensible converts the example JSON output from the [example document](https://raw.githubusercontent.com/sensible-hq/sensible-docs/main/readme-sync/assets/v0/pdfs/vertical%5Fsection%5Ftable%5Fin%5Ftable.pdf) described in the [Advanced Sections nested table example](/senseml-reference/sections/sections-example-nested-table) topic to the following spreadsheet:

<iframe width="700" src="https://docs.google.com/spreadsheets/d/e/2PACX-1vTVfCgSQir-GkJsYrDv3TBlHcuH11YPt9P3CGXp9gHFnrFoopKEVz0wQ2jPhezpiE1uHip08LqO7lmV/pubhtml?widget=true&amp;headers=false"></iframe>
...
See the [Advanced sections nested table example](/senseml-reference/sections/sections-example-nested-table) topic for the SenseML configuration for this example.
...
* For a tutorial about converting document data to an Excel file using the Sensible app, see [Getting started with out-of-the-box extraction](/layout-based-extractions/library-quickstart).
