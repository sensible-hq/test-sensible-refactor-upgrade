...
* You extract from a [portfolio](/layout-based-extractions/multi-document-extractions) document.
* You input multiple extractions to the Generate Excel [method](/integrations/sdk-guides) or [endpoint](/reference/get-excel-extraction).
...
* To configure multi-document spreadsheet output using different rules than those described here, configure rules with Sensible's [Zapier](/integrations/zapier-overview/overview) integration. You can map single-value field output with this integration, but not multi-value field output.
...
See the [Getting started with layout-based extractions](/welcome/getting-started) for the SenseML configuration for this example.
...
* the [table methods](/senseml-reference/concepts/choosing-a-table-method) and other methods that output nested JSON objects.
...
Each `<field_id>.<index>` sheet lists the output of a single field that contains complex repeating data, for example, [Sections](/senseml-reference/sections/overview) output.

### Example

Sensible converts the example JSON output from the [example document](https://raw.githubusercontent.com/sensible-hq/sensible-docs/main/readme-sync/assets/v0/pdfs/vertical%5Fsection%5Ftable%5Fin%5Ftable.pdf) described in the [Advanced Sections nested table example](/senseml-reference/sections/advanced--nested-table-example) topic to the following spreadsheet:
...
See the [Advanced sections nested table example](/senseml-reference/sections/advanced--nested-table-example) topic for the SenseML configuration for this example.
...
* For a tutorial about converting document data to an Excel file using the Sensible app, see [Getting started with out-of-the-box extraction](/layout-based-extractions/out-of-the-box-extractions).
* To get started with extracting from your custom documents, see [Getting Started](/welcome/getting-started).