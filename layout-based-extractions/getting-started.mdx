...
* SenseML is for advanced config authoring. For a simpler authoring experience, use Sensible Instruct. For more information about SenseML versus Sensible Instruct, see [Choosing extraction strategy](/welcome/author). For authoring in Sensible Instruct, see [Getting started](/welcome/getting-started-ai).
...
* If you want a quick "hello world" API response, see the [API quickstart](/integrations/quickstart).
...
* [Write a collection of queries ( a "config")](/layout-based-extractions/getting-started#create-a-config) to [extract structured data](/layout-based-extractions/getting-started#extract-data) from an example auto insurance document
* [Learn how the config works](/layout-based-extractions/getting-started#how-it-works), including key concepts like lines, anchors, and methods
* [Test the config](/layout-based-extractions/getting-started#test-the-config) by running your config against a second, similar auto insurance document
* [Use the API](/layout-based-extractions/getting-started#integrate-with-your-application) to integrate your Sensible config with your application
* [Validate extractions in production](/layout-based-extractions/getting-started#validate-extractions-in-production) by using JsonLogic to define expected extracted values and flag unexpected values as warnings or errors
...
![Click to enlarge](/assets/v0/images/screenshots/quickstart_doc_type.png)
...
| Example document | [Download link](https://github.com/sensible-hq/sensible-docs/raw/main/readme-sync/assets/v0/pdfs/auto%5Finsurance%5Fanyco.pdf) |  
   ...
   ![Click to enlarge](/assets/v0/images/screenshots/quickstart_upload_doc.png)  
   2. In the file upload dialog, choose the generic car insurance quote you downloaded in a previous step.
3. To create a configuration for your document type, take the following steps:  
...
4. To edit your **anyco** configuration, click it. When the configuration opens, you see an empty config pane on the left, the document in the middle, and an empty output pane on the right:

![Click to enlarge](/assets/v0/images/screenshots/quickstart_blank_config.png)
...
![Click to enlarge](/assets/v0/images/screenshots/quickstart_after_paste.png)
...
* For a deep dive on how the config works, see [the following section](/layout-based-extractions/getting-started#how-it-works).
* If you want to skip ahead and try out the API, see [Integrate with your application](/layout-based-extractions/getting-started#integrate-with-your-application).

## How layout-based extraction works

This guide focuses on layout-based document extraction, which works as follows:

* Each "field" is a basic query unit in Sensible. Each field outputs a piece of data from the document that you want to extract. Sensible uses the field `id` as the key in the key/value JSON output. For more information, see [Field](/senseml-reference/field-query-object/index-field-query-object).
* Sensible searches first for a text "anchor" because it's a computationally quick way to narrow down the location of the target data to extract. An anchor is text that always occurs close to your target text. Without it, Sensible wouldn't know which page to search in for your target text . For more information about defining complex anchors, see [Anchor](/senseml-reference/field-query-object/anchor).
* Then, Sensible uses a "method" to expand its search out from the anchor and extract the data you want. For more information about methods, see [Methods](/senseml-reference/methods/index-methods).

This config uses three types of layout-based methods:

| Type of method | explanation                                                                   | description                                       |
| -------------- | ----------------------------------------------------------------------------- | ------------------------------------------------- |
| layout         | [How it works: label method](/layout-based-extractions/getting-started#how-it-works-label-method) | Grab info immediately proximate to labeling text. |
| layout         | [How it works: row method](/layout-based-extractions/getting-started#how-it-works-row-method)     | Grab info from a cell in a row.                   |
| layout         | [How it works: box method](/layout-based-extractions/getting-started#how-it-works-box-method)     | Grab info from a box.                             |
...
| Type of method   | explanation                                                                   | description                                                       |
| ---------------- | ----------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| Natural-language | [How it works: query method](/layout-based-extractions/getting-started#how-it-works-query-method) | Ask a free-text question about simple information in the document |
...
![Click to enlarge](/assets/v0/images/screenshots/quickstart_question.png)

The config uses the [Query Group](/senseml-reference/llm-based-methods/query-group) method to query for the `bodily injury premium`. You can group together other queries if the answers are located within a page or two of each other in the document. For example, in the group, the config also queries for the `insurer's customer service phone number`.
...
```
Try it out: change one of the questions to `"street address for the Anyco insurance company"` and see what you get. For easy authoring, try out this method in Sensible's [visual authoring tool](/llm-based-extractions/prompt-tips/index-prompt-tips).
...
![Click to enlarge](/assets/v0/images/screenshots/quickstart_label_right.png)

The config uses the [Label method](/senseml-reference/methods/label):
...
![Click to enlarge](/assets/v0/images/screenshots/quickstart_concept_1.png)
...
![Click to enlarge](/assets/v0/images/screenshots/quickstart_concept_2.png)
...
![Click to enlarge](/assets/v0/images/screenshots/quickstart_row.png)

The config uses the [Row method](/senseml-reference/methods/row):
...
* The returned value is a currency (`"type": "currency"`). For other data types you can define, see [Field query object](/senseml-reference/field-query-object/index-field-query-object).
...
![Click to enlarge](/assets/v0/images/screenshots/quickstart_concept_3.png)
...
![Click to enlarge](/assets/v0/images/screenshots/quickstart_box.png)

The config uses the [Box method](/senseml-reference/methods/box):
...
* The anchor line is a little more complex than previous examples, because it also defines a match type (`"type": "startsWith"`). You can write a simpler string anchor as `"anchor":"policy number"`, or you can expand to complex anchors. For more information, see [Anchor object](/senseml-reference/field-query-object/anchor).
...
* You can use a [Column method](/senseml-reference/methods/column) to return all the listed premiums ($90, $15, $130).
* The limits listed in the table are tricky for the Row method to capture since they can be a variable number of lines. Row methods depend on strict horizontal alignment of lines, so Sensible extracts the first line. Instead, use the [Table method](/senseml-reference/concepts/table-methods) to more reliably capture the data in each cell of the whole table. Or, use an `xRangeFilter` parameter in the [Document Range method](/senseml-reference/methods/document-range) to capture the limits.
* What if the document listed emails, and you just wanted to capture all those emails? You could use a regular expression (regex) in a `"match":"all"` anchor coupled with a [Passthrough method](/senseml-reference/methods/passthrough), or the [Regex method](/senseml-reference/methods/regex).
* You can split the policy period into two dates, either by using the [Split computed field method](/senseml-reference/computed-field-methods/split), or by setting the [Date](/senseml-reference/field-query-object/types#date) type on the field and using a tiebreaker.

To check out other methods, see [Methods](/senseml-reference/methods/index-methods).

## Test the config

Before integrating the config with an application and writing [validation tests](/best-practices/validate-extractions) against it, double check the config by uploading another quote.
...
```
![Click to enlarge](/assets/v0/images/screenshots/quickstart_error_1.png)
...
You can use the [Document Range](/senseml-reference/methods/document-range) method to extract the policy period. This method extracts succeeding lines of text after an anchor. You need to configure some optional parameters, because the Document Range method by default discards anchor lines. Since the date range is part of the anchor line (the line containing `"policy period"`), you need to specify to:
...
You can use the [Region method](/senseml-reference/methods/region) to extract the policy period. A region is a rectangular space defined by coordinates relative to the anchor. 
...
![Click to enlarge](/assets/v0/images/screenshots/quickstart_error_3.png)

Let's double check that this region also works with the first document:

![Click to enlarge](/assets/v0/images/screenshots/quickstart_error_4.png)
...
1. Click **Publish configuration**. The config is still a work in progress, so click **Development**. Now you can use the query parameter `env=development` to test the integration before you go to production:![Click to enlarge](/assets/v0/images/screenshots/quickstart_publish_config.png).
...
```
![Click to enlarge](/assets/v0/images/screenshots/quickstart_validation.png)
...
```
5. To test the validations with a document that's missing information, [try out an API call](/api-guides/api-tutorial/api-tutorial-async-1) with the following example document that has these errors:  
...
* Check out the [SenseML method reference docs](/senseml-reference/methods/index-methods) to write your own extractions
* Learn more about [validations](/best-practices/validate-extractions) to test the quality of your extractions in production