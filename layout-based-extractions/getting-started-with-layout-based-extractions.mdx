...
* If you want a quick "hello world" API response, see the [API quickstart](/integrations/api-quickstart).
...
* [Write a collection of queries ( a "config")](/layout-based-extractions/getting-started-with-layout-based-extractions#create-a-config) to [extract structured data](/layout-based-extractions/getting-started-with-layout-based-extractions#extract-data) from an example auto insurance document
* [Learn how the config works](/layout-based-extractions/getting-started-with-layout-based-extractions#how-it-works), including key concepts like lines, anchors, and methods
* [Test the config](/layout-based-extractions/getting-started-with-layout-based-extractions#test-the-config) by running your config against a second, similar auto insurance document
* [Use the API](/layout-based-extractions/getting-started-with-layout-based-extractions#integrate-with-your-application) to integrate your Sensible config with your application
* [Validate extractions in production](/layout-based-extractions/getting-started-with-layout-based-extractions#validate-extractions-in-production) by using JsonLogic to define expected extracted values and flag unexpected values as warnings or errors
...
* For a deep dive on how the config works, see [the following section](/layout-based-extractions/getting-started-with-layout-based-extractions#how-it-works).
* If you want to skip ahead and try out the API, see [Integrate with your application](/layout-based-extractions/getting-started-with-layout-based-extractions#integrate-with-your-application).

## How layout-based extraction works

This guide focuses on layout-based document extraction, which works as follows:

* Each "field" is a basic query unit in Sensible. Each field outputs a piece of data from the document that you want to extract. Sensible uses the field `id` as the key in the key/value JSON output. For more information, see [Field](/senseml-reference/field-query-object/overview).
* Sensible searches first for a text "anchor" because it's a computationally quick way to narrow down the location of the target data to extract. An anchor is text that always occurs close to your target text. Without it, Sensible wouldn't know which page to search in for your target text . For more information about defining complex anchors, see [Anchor](/senseml-reference/field-query-object/anchor-object).
* Then, Sensible uses a "method" to expand its search out from the anchor and extract the data you want. For more information about methods, see [Methods](/senseml-reference/methods/overview).

This config uses three types of layout-based methods:

| Type of method | explanation                                                                   | description                                       |
| -------------- | ----------------------------------------------------------------------------- | ------------------------------------------------- |
| layout         | [How it works: label method](/layout-based-extractions/getting-started-with-layout-based-extractions#how-it-works-label-method) | Grab info immediately proximate to labeling text. |
| layout         | [How it works: row method](/layout-based-extractions/getting-started-with-layout-based-extractions#how-it-works-row-method)     | Grab info from a cell in a row.                   |
| layout         | [How it works: box method](/layout-based-extractions/getting-started-with-layout-based-extractions#how-it-works-box-method)     | Grab info from a box.                             |
...
| Type of method   | explanation                                                                   | description                                                       |
| ---------------- | ----------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| Natural-language | [How it works: query method](/layout-based-extractions/getting-started-with-layout-based-extractions#how-it-works-query-method) | Ask a free-text question about simple information in the document |
...
* The returned value is a currency (`"type": "currency"`). For other data types you can define, see [Field query object](/senseml-reference/field-query-object/overview).
...
To check out other methods, see [Methods](/senseml-reference/methods/overview).

## Test the config

Before integrating the config with an application and writing [validation tests](/best-practices/validating-extractions) against it, double check the config by uploading another quote.
...
```
5. To test the validations with a document that's missing information, [try out an API call](/api/api-tutorial/try-asynchronous-extraction-from-your-url) with the following example document that has these errors:  
...
* Check out the [SenseML method reference docs](/senseml-reference/methods/overview) to write your own extractions
* Learn more about [validations](/best-practices/validating-extractions) to test the quality of your extractions in production